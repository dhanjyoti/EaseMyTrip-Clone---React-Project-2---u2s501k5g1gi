...
import TravallerClass from '../components/travaller-class/TravallerClass';
...
const TripTab = ({onChange, active}) =>{
  ...
  return <div className='flex flex-row items-center'>
    {items.map((item)=><div key={item.value} className={classNames('cursor-pointer text-xs font-bold px-2 py-1 rounded-full hover:bg-white hover:text-[#1853a2]', active == item.value ? "bg-white text-[#1853a2]":"text-[#cad5f5]")} onClick={()=>onChange?.(item.value)}>{item.label}</div>)}
  </div>
}
const SearchBar = () => {
  const fromPoint = [
